...
st.set_page_config(
    page_title="Ocorrências de Roubo no Comércio - ES",
    page_icon="🚨",
    layout="wide"
)
...
arquivo = st.file_uploader(
    "Envie o arquivo CSV das ocorrências",
    type=["csv"]
)
...
if arquivo is not None:
    ...
    try:
        ...
        ocorrencias_ano = ocorrencias_ano.dropna(
            subset=["ANO"]
        )
        ...
        fig1 = px.line(
            ...
            y="Quantidade",
            markers=True,
            title="Ocorrências de roubo por ano",
            labels={
                "ANO": "Ano",
        ...
        if not ocorrencias_ano.empty:
            ...
            with col1:
                ...
                st.write(
                    f"Ocorrências: **{int(maior_ano['Quantidade'])}**"
                )
            ...
            with col2:
                ...
                st.write(
                    f"Ocorrências: **{int(menor_ano['Quantidade'])}**"
                )
        ...
        if not municipios.empty:
            ...
            maior_municipio = municipios.iloc[0]

            st.metric(
                "Município com mais ocorrências",
                maior_municipio["Municipio"],
                f"{int(maior_municipio['Quantidade'])} ocorrências"
            )
        ...
        fig5 = px.pie(
            ...
            names="Local",
            values="Quantidade",
            title="Distribuição por tipo de local"
        )
    ...
    st.info(
        "Envie o arquivo CSV para começar."
    )